...
import { announcementsService, usersService } from '../../services';
...
const AnnouncementsPostsTab = ({ currentUser, theme }) => {
  ...
  // Role-based access control functions - Only admin can create/edit/delete
  const canCreatePost = () => {
  ...
  const loadPosts = async () => {
    try {
      setLoading(true);
      const postsData = await announcementsService.getAnnouncementPosts();
      let processedPosts = [];
  ...
  const handleCreateRichPost = async () => {
    if (postSubject.trim() && postMessage.trim() && !loadingStates.creatingPost) {
      ...
      try {
        ...
        const newPost = await announcementsService.createAnnouncementPost(postData, fileToUpload);
    ...
    if (!loadingStates.deletingPost) {
      showConfirmation({
        ...
        onConfirm: async () => {
          ...
          try {
            await announcementsService.deleteAnnouncementPost(postId);
  ...
  const handleUpdatePost = async () => {
    if (editPostMessage.trim() && editingPost && !loadingStates.updatingPost) {
      ...
      try {
        ...
        await announcementsService.updateAnnouncementPost(editingPost.id, updateData, fileToUpload);
        ...
        showErrorToast(error, 'Failed to update announcement. Please try again.');
      } finally {
  ...
  const handleEditFileAttachment = (event) => {
    ...
    if (files.length > 1) {
      showErrorToast(null, 'Only one file can be attached per announcement. Please select a single file.');
      return;
    }
  ...
  const handleRemoveEditAttachment = async (attachmentId) => {
    try {
      ...
      if (attachment && attachment.isExisting) {
        ...
        await announcementsService.deleteAnnouncementAttachment(attachmentId);
  ...
  const handleDownloadFile = async (attachment) => {
    try {
      ...
      const blob = await announcementsService.downloadAnnouncementAttachment(attachment.id);
  ...
  const handleOpenFile = async (attachment) => {
    try {
      ...
      console.log('👁️ Opening file in AnnouncementsPostsTab:', fileName);
            ...
            <div className="mb-6">
              <div className="relative w-32 h-32 mx-auto">
                ...
                {currentUser?.role === 'parent' && (
                  <>
                    <div className="absolute top-0 left-0 w-16 h-16 bg-purple-500 rounded-full opacity-80 animate-pulse"></div>
                    <div className="absolute top-4 right-0 w-16 h-16 bg-purple-400 rounded-full opacity-80 animate-pulse"></div>
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 h-16 bg-purple-300 rounded-full opacity-80 animate-pulse"></div>
                  </>
                )}
              </div>
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">Loading announcements...</h3>
          ...
          <div className="text-center py-12">
            <div className="mb-6">
              <div className="relative w-32 h-32 mx-auto">
                ...
                {currentUser?.role === 'parent' && (
                  <>
                    <div className="absolute top-0 left-0 w-16 h-16 bg-purple-500 rounded-full opacity-80"></div>
                    <div className="absolute top-4 right-0 w-16 h-16 bg-purple-400 rounded-full opacity-80"></div>
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 h-16 bg-purple-300 rounded-full opacity-80"></div>
                  </>
                )}
              </div>
            </div>
              ...
              <div key={post.id} className="bg-white rounded-lg p-4 border border-gray-200 hover:shadow-sm transition-shadow">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div 
                        className={`w-10 h-10 ${(post.authorId === currentUser?.id ? currentUser?.role : post.author?.role) === 'admin' ? 'bg-green-600' : 'bg-blue-500'} rounded-full flex items-center justify-center flex-shrink-0`}
                      >
          ...
          <div className="bg-white rounded-lg p-6 w-11/12 max-w-4xl shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">Editing announcement</p>
              </div>
            </div>
...
export default AnnouncementsPostsTab;
